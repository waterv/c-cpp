...
int main() {
  ...
  ::RegisterClassEx(&wc);
  HWND hwnd =
      ::CreateWindow(wc.lpszClassName, _T("Puzzles 52"), WS_OVERLAPPEDWINDOW,
  ...
  ImGui::GetStyle().ItemSpacing = {4, 4};
  ...
  while (!done) {
    ...
    if (show_akari_window) AkariWindow(&show_akari_window);

    {
      ...
      ImGui::Checkbox("Akari Window", &show_akari_window);
      ImGui::Checkbox("Another Window", &show_another_window);

      ImGui::SliderFloat("float", &f, min, max);
      ImGui::SliderFloat("Minimum", &min, 0.0f, 1.0f);
      ImGui::SliderFloat("Maximum", &max, 0.0f, 1.0f);
